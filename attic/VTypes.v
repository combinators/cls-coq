...
Section IntersectionTypeSchemas.
  (** The set of type constructors. **)
  Variable Constructor: countType.

  (** The set of type variables **)
  Variable TypeVariable: countType.

  (** Intersection types with constructors and products. **)
  Inductive ITS : Type :=
  | Omega : ITS
  | Ctor : Constructor -> ITS -> ITS
  | Arrow : ITS -> ITS -> ITS
  | Prod : ITS -> ITS -> ITS
  | Inter : ITS -> ITS -> ITS
  | Var : TypeVariable -> ITS.

  (** Enable mathcomp functionalities on Intersection Type Schemas **)
  Section ITSMathcompInstances.
    Fixpoint ITS2tree (A: ITS): GenTree.tree (Constructor + TypeVariable) :=
      match A with
      | Omega => GenTree.Node 0 [::]
      | Ctor C A => GenTree.Node 1 [:: GenTree.Leaf (inl C); ITS2tree A  ]
      | Arrow A1 A2 => GenTree.Node 2 [:: ITS2tree A1; ITS2tree A2 ]
      | Prod A1 A2 => GenTree.Node 3 [:: ITS2tree A1; ITS2tree A2 ]
      | Inter A1 A2 => GenTree.Node 4 [:: ITS2tree A1; ITS2tree A2 ]
      | Var v => GenTree.Node 5 [:: GenTree.Leaf (inr v) ]
      end.

    Fixpoint tree2ITS (t: GenTree.tree (Constructor + TypeVariable)): option ITS :=
      match t with
      | GenTree.Node n args =>
        match n, args with
        | 0, [::] => Some Omega
        | 1, [:: GenTree.Leaf (inl C);  t] => option_map (Ctor C) (tree2ITS t)
        | 2, [:: t1; t2] => if tree2ITS t1 is Some A1 then
                             if tree2ITS t2 is Some A2 then Some (Arrow A1 A2) else None
                           else None
        | 3, [:: t1; t2] => if tree2ITS t1 is Some A1 then
                             if tree2ITS t2 is Some A2 then Some (Prod A1 A2) else None
                           else None
        | 4, [:: t1; t2] => if tree2ITS t1 is Some A1 then
                             if tree2ITS t2 is Some A2 then Some (Inter A1 A2) else None
                           else None
        | 5, [:: GenTree.Leaf (inr v)] => Some (Var v)
        | _, _ => None
        end
      | _ => None
      end.

    Lemma pcan_ITStree: pcancel ITS2tree tree2ITS.
    Proof.
      elim => //=; by [ move => ? -> ? -> || move => ? ? -> ].
    Qed.

    Definition ITS_eqMixin := PcanEqMixin pcan_ITStree.
    Canonical ITS_eqType := EqType ITS ITS_eqMixin.
    Definition ITS_choiceMixin := PcanChoiceMixin pcan_ITStree.
    Canonical ITS_choiceType := ChoiceType ITS ITS_choiceMixin.
    Definition ITS_countMixin := PcanCountMixin pcan_ITStree.
    Canonical ITS_countType := CountType ITS ITS_countMixin.
  End ITSMathcompInstances.

  (** Check if a type is Omega or an Arrow ending in Omega **)
  Fixpoint isOmega (A: ITS) {struct A}: bool :=
    match A with
    | Omega => true
    | Arrow A B => isOmega B
...
Arguments IT [Constructor TypeVariable].
Arguments Omega [Constructor TypeVariable].
Arguments Ctor [Constructor TypeVariable].
Arguments Arrow [Constructor TypeVariable].
Arguments Prod [Constructor TypeVariable].
Arguments Inter [Constructor TypeVariable].
Arguments Var [Constructor TypeVariable].
Hint Constructors IT.

Arguments isOmega [Constructor TypeVariable].
Arguments arity [Constructor TypeVariable].
Arguments omegaArg [Constructor TypeVariable].
Arguments intersect [Constructor TypeVariable].
...
Notation "\prod_ ( i <- xs ) F" :=
  (\big[(@Prod _ _)/(last Omega xs)]_(i <- behead (belast Omega xs)) F).
...
Lemma bigcap_cons: forall (Constructor: countType) (TypeVariable: countType) (T: Type) (F: T -> @IT Constructor TypeVariable) (A: T) (Delta: seq T),
    \bigcap_(A__i <- [:: A & Delta]) (F A__i) =
    match Delta with
    | [::] => F A
...
Proof. 
  move => ? ? T F A Delta.
    by case: Delta.
Qed.
Arguments bigcap_cons [Constructor TypeVariable T F A Delta].
...
Section BCD.
  ...
  Inductive BCD: @IT Constructor TypeVariable -> @IT Constructor TypeVariable -> Prop :=
...
Arguments BCD [Constructor TypeVariable].
Arguments BCD__CAx [Constructor TypeVariable] [a b A B].
Arguments BCD__omega [Constructor TypeVariable] [A].
Arguments BCD__CDist [Constructor TypeVariable] [a A1 A2].
Arguments BCD__ArrOmega [Constructor TypeVariable].
Arguments BCD__Sub [Constructor TypeVariable] [A1 A2 B1 B2].
Arguments BCD__Dist [Constructor TypeVariable] [A B1 B2].
Arguments BCD__Idem [Constructor TypeVariable] [A].
Arguments BCD__Trans [Constructor TypeVariable] [A] B [C].
Arguments BCD__Glb [Constructor TypeVariable] [A B1 B2].
Arguments BCD__Lub1 [Constructor TypeVariable] [A B].
Arguments BCD__Lub2 [Constructor TypeVariable] [A B].
Arguments BCD__Refl [Constructor TypeVariable] [A].
